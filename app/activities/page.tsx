    ...
    <section className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <form className="mt-6 space-y-4" onSubmit={saveActivity}>
          ...
          <div className="space-y-3"><div className="flex items-center justify-between"><p className="text-sm font-semibold">Words and phonemes</p><button type="button" onClick={() => { setForm({ ...form, words: [...form.words, blankWord()] }); setPhonemeDrafts([...phonemeDrafts, '']); }} className="rounded-full border border-sky-300 px-3 py-1 text-sm text-sky-700 transition hover:border-sky-500 hover:bg-sky-50 hover:text-sky-800 dark:hover:bg-sky-950/40">Add word</button></div>{form.words.map((word, index) => <div key={index} className="rounded-2xl border border-slate-200 p-3 dark:border-slate-700"><div className="mb-2 flex justify-end"><button type="button" onClick={() => removeWord(index)} aria-label={`Remove word ${index + 1}`} className="rounded-full px-2 py-1 text-sm font-medium text-red-700 underline transition hover:bg-red-50 hover:text-red-900 dark:text-red-400 dark:hover:bg-red-950/40 dark:hover:text-red-300">Remove</button></div><input required aria-label={`Phoneme transcription for word ${index + 1}`} placeholder="Phoneme transcription" value={word.phonemeWord} onChange={(event) => updatePhonemeWord(index, event.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800" /><input required aria-label={`English word for word ${index + 1}`} placeholder="English word" value={word.englishEquivalence} onChange={(event) => updateWord(index, 'englishEquivalence', event.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800" /><input required aria-label={`Phoneme symbols for word ${index + 1}`} placeholder="Phoneme symbols, separated by commas" value={phonemeDrafts[index] ?? word.phonemes.join(', ')} onChange={(event) => updatePhonemesDraft(index, event.target.value)} onBlur={() => normalisePhonemes(index)} className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800" /></div>)}</div>
          <div className="flex gap-2"><button type="submit" className="rounded-full bg-sky-600 px-4 py-2 font-medium text-white transition hover:bg-sky-700 hover:shadow-md">{editingId ? 'Update activity' : 'Save activity'}</button>{editingId && <button type="button" onClick={() => { setEditingId(null); setForm(blankForm); }} className="rounded-full border border-slate-300 px-4 py-2 transition hover:border-slate-400 hover:bg-slate-100 dark:hover:border-slate-600 dark:hover:bg-slate-800">Cancel</button>}</div>
          {message && <p className="text-sm text-emerald-700">{message}</p>}{error && <p className="text-sm text-red-700">{error}</p>}
        </form>
      </div>
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-600">Stored records</p><h2 className="mt-1 text-2xl font-semibold">Saved activities</h2></div><a href="/api/health" target="_blank" rel="noreferrer" className="text-sm text-sky-700 underline transition hover:text-sky-900 dark:text-sky-400 dark:hover:text-sky-300">Health API</a></div><div className="mt-5 space-y-3">{activities.map((activity) => <article key={activity.id} className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{activity.title}</h3><p className="text-sm text-slate-500">{formatActivityType(activity.activityType)}{activity.activityType === 'WORD_SEARCH' ? ` · ${formatDifficulty(activity.difficulty)}` : ''} · {activity.words.length} words</p></div><div className="flex gap-2"><button type="button" onClick={() => editActivity(activity)} className="rounded-full px-2 py-1 text-sm font-medium text-sky-700 transition hover:bg-sky-50 hover:text-sky-900 dark:text-sky-400 dark:hover:bg-sky-950/40 dark:hover:text-sky-300">Edit</button><button type="button" onClick={() => deleteActivity(activity.id)} className="rounded-full px-2 py-1 text-sm font-medium text-red-700 transition hover:bg-red-50 hover:text-red-900 dark:text-red-400 dark:hover:bg-red-950/40 dark:hover:text-red-300">Delete</button></div></div><ul className="mt-3 flex flex-wrap gap-2">{activity.words.map((word) => <li key={word.id ?? word.phonemeWord} className="rounded-full bg-slate-100 px-3 py-1 text-sm dark:bg-slate-800">{word.phonemeWord} · {word.englishEquivalence}</li>)}</ul></article>)}{activities.length === 0 && <p className="text-sm text-slate-500">No saved activities yet.</p>}</div></div>
    </section>
  );
}
